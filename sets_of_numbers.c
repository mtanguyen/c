...
Name of file: sets_of_numbers.c
Purpose: Sets are represented as arrays. The zeroth element of each array has a special meaning: it indicates the number of elements
in the set. The set elements themselves will be located in array elements 1 through N. 
Author: My Nguyen
...
/*read_set reads integes from standard input and uses enter_set to add them to the set avoiding repeated entries. */
void read_set(int a[])
{
...
/*print_set prints out the content of the set, in the traditional mathemetical notation, as a list of items separated by commas, surrounded by braces. */
...
/*delete_set deletes an element in set A. */
void delete_set(int v, int a[])
{
   int i, p;
   
// search for v and delete all vs found
   for (i = 1; i <= a[0]; i++)
   {
	  if (a[i] == v)
	  {
	  	for(p = i; p < a[0]; p++)
			{  
        a[p] = a[p + 1]; //position needs to be shifted
      }
      a[0]--; // decrease to take away v 
	   }
   }  
}

/*set_difference carries out the set difference operation on set a and b, storing the result in set c. */
void set_difference(int a[], int b[], int c[])
{
   int i;
   c[0] = 0;  //c[0] is a special elements. It indicates the number of elements in set c.

// serach for all elements in a not in b
   for(i = 1; i <= a[0]; i++)   
     if (!element_of(a[i], b))
       enter_set(a[i], c);  
}

/*sym_difference calculates the symmetric difference between set a and b (set of all of elements in either a or b, but not both), storing the result in set c. */
void sym_difference(int a[], int b[], int c[])
{

// symmetric difference is union - intersection
  int i[MAX_SET_SIZE], j[MAX_SET_SIZE];
  union_set(a, b, j); 
  intersect_set(a, b, i); 
  set_difference(j, i, c); 
